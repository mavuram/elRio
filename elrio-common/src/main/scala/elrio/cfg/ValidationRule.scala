...
abstract class  ValidationRule(op: Op, vals: Array[AnyRef])

abstract class SimpleRule(op: Op, operand: String, vals: Array[AnyRef]) extends ValidationRule(op, vals)
abstract class CompositeRule(op: Op, vals: Array[AnyRef]) extends ValidationRule(op, vals)
...
case class Or @JsonCreator()        (@JsonProperty("op") op: String, @JsonProperty("lrule") lRule: ValidationRule, @JsonProperty("rrule") rRule: ValidationRule)   extends CompositeRule(OR, Array(lRule, rRule))
case class And @JsonCreator()       (@JsonProperty("op") op: String, @JsonProperty("lrule") lRule: ValidationRule, @JsonProperty("rrule") rRule: ValidationRule)   extends CompositeRule(AND, Array(lRule, rRule))
case class Not @JsonCreator()       (@JsonProperty("op") op: String, @JsonProperty("rule") rule: ValidationRule)                                         extends CompositeRule(NOT, Array(rule))

case class Rule @JsonCreator()(@JsonProperty("rule_id")   ruleId: Int,
                               @JsonProperty("rule")      rule: ValidationRule,
